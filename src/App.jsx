...
const deadlineTypes = [
  { id: 'ed', label: 'Early Decision', color: '#ff4b4b', bg: '#ffdfe0' },
  { id: 'ea', label: 'Early Action', color: '#ffc800', bg: '#fff4d4' },
  { id: 'rd', label: 'Regular Decision', color: '#1cb0f6', bg: '#ddf4ff' },
  { id: 'scholarship', label: 'Scholarship', color: '#ce82ff', bg: '#f3e5ff' },
  { id: 'interview', label: 'Interview', color: '#58cc02', bg: '#d7ffb8' },
  { id: 'other', label: 'Other', color: '#777', bg: '#f7f7f7' },
]
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      setSession(session)
      if (session) loadUserData(session.user.id)
      else { setDataLoaded(false); setDeadlines([]); setWeeklyGoals([]); setWeeklyTasks({}); setDailyChecks({}); setWeeklyChecks({}) }
    })
  ...
  const saveDeadline = (form) => { if (!form.title.trim()) return; const deadline = { id: editingDeadline ? editingDeadline.id : Date.now(), date: formatDateId(selectedDate), ...form }; if (editingDeadline) { setDeadlines(deadlines.map(d => d.id === editingDeadline.id ? deadline : d)) } else { setDeadlines([...deadlines, deadline]) } setShowModal(false); setEditingDeadline(null) }
  const deleteDeadline = (id) => { setDeadlines(deadlines.filter(d => d.id !== id)); setShowModal(false) }
  ...
  const toggleDaily = (taskId) => { const key = getTodayId() + '-' + taskId; setDailyChecks({ ...dailyChecks, [key]: !dailyChecks[key] }) }
  const toggleWeekly = (taskId) => { const key = currentWeekId + '-' + taskId; setWeeklyChecks({ ...weeklyChecks, [key]: !weeklyChecks[key] }) }
  ...
  return (
    <div style={styles.app}>
      ...
      <div style={styles.content}>
        {screen === 'calendar' && <CalendarScreen calendarDate={calendarDate} setCalendarDate={setCalendarDate} deadlines={deadlines} onSelectDate={(date, dl) => { setSelectedDate(date); setEditingDeadline(dl || null); setShowModal(true) }} />}
...
function CalendarScreen({ calendarDate, setCalendarDate, deadlines, onSelectDate }) {
    ...
    <div>
      ...
      <div style={styles.calendarCard}>
        ...
        <div style={styles.calGrid}>
          ...
          {days.map((day, i) => { const dayDeadlines = getDeadlinesForDay(day); return <div key={i} style={{ ...styles.calDay, ...(isToday(day) ? styles.calDayToday : {}), visibility: day ? 'visible' : 'hidden', cursor: day ? 'pointer' : 'default' }} onClick={() => day && onSelectDate(new Date(year, month, day), dayDeadlines[0])}><span style={{ fontWeight: isToday(day) ? 700 : 500 }}>{day}</span>{dayDeadlines.length > 0 && <div style={styles.calDots}>{dayDeadlines.slice(0, 3).map((d, j) => { const type = deadlineTypes.find(t => t.id === d.type); return <div key={j} style={{ width: 6, height: 6, borderRadius: '50%', background: type ? type.color : '#777' }} /> })}</div>}</div> })}
        </div>
      </div>
      <div style={{ marginTop: 24 }}><h3 style={styles.sectionTitle}>Upcoming Deadlines</h3>
        {upcoming.length === 0 ? <div style={styles.emptyState}><p style={{ fontSize: 32 }}>📝</p><p style={{ color: '#777' }}>No deadlines yet</p><p style={{ color: '#58cc02', fontWeight: 600, fontSize: 14, marginTop: 8 }}>Click any date above to add one!</p></div> : upcoming.map(deadline => { const type = deadlineTypes.find(t => t.id === deadline.type); const date = new Date(deadline.date + 'T00:00:00'); const daysLeft = Math.ceil((date - new Date()) / (1000 * 60 * 60 * 24)); return <div key={deadline.id} style={{ ...styles.deadlineCard, background: type ? type.bg : '#f7f7f7' }} onClick={() => onSelectDate(date, deadline)}><div style={{ flex: 1 }}><span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', color: type ? type.color : '#777' }}>{type ? type.label : 'Other'}</span><h4 style={{ fontSize: 16, fontWeight: 700, marginTop: 4 }}>{deadline.title}</h4>{deadline.college && <p style={{ fontSize: 14, color: '#777' }}>{deadline.college}</p>}</div><div style={{ textAlign: 'right' }}><p style={{ fontSize: 14, fontWeight: 600 }}>{date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</p><p style={{ fontSize: 12, color: daysLeft <= 7 ? '#ff4b4b' : '#777', fontWeight: daysLeft <= 7 ? 700 : 400 }}>{daysLeft} days</p></div></div> })}
      </div>
    </div>
  )
}
...
function DeadlineModal({ date, deadline, onSave, onDelete, onClose }) {
  const [form, setForm] = useState({ title: deadline ? deadline.title : '', type: deadline ? deadline.type : 'rd', college: deadline ? deadline.college : '', notes: deadline ? deadline.notes : '' })
  return (
    <div style={styles.modalOverlay} onClick={onClose}>
      <div style={styles.modal} onClick={e => e.stopPropagation()}>
        <h2 style={{ fontFamily: 'Nunito, sans-serif', fontSize: 22, fontWeight: 800, marginBottom: 8 }}>{deadline ? 'Edit Deadline' : 'Add Deadline'}</h2>
        ...
        <input style={styles.modalInput} placeholder="Deadline title" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} />
        <input style={styles.modalInput} placeholder="College name (optional)" value={form.college} onChange={e => setForm({ ...form, college: e.target.value })} />
        <div style={{ marginBottom: 16 }}><label style={{ fontSize: 12, fontWeight: 600, color: '#777', marginBottom: 8, display: 'block' }}>Type</label><div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>{deadlineTypes.map(type => <button key={type.id} style={{ padding: '8px 14px', borderRadius: 20, border: form.type === type.id ? '2px solid ' + type.color : '2px solid #e5e5e5', background: form.type === type.id ? type.bg : '#fff', color: type.color, fontSize: 12, fontWeight: 600 }} onClick={() => setForm({ ...form, type: type.id })}>{type.label}</button>)}</div></div>
        ...
        <div style={{ display: 'flex', gap: 12 }}>{deadline && <button style={{ ...styles.secondaryBtn, flex: 1, color: '#ff4b4b' }} onClick={() => onDelete(deadline.id)}>Delete</button>}<button style={{ ...styles.secondaryBtn, flex: 1 }} onClick={onClose}>Cancel</button><button style={{ ...styles.primaryBtn, flex: 1 }} onClick={() => onSave(form)}>Save</button></div>
      </div>
    </div>
  )
}
...
function PlanningScreen({ dateRange, deadlines, currentGoals, currentTasks, currentWeekId, expandedGoal, setExpandedGoal, addGoal, removeGoal, addTask, removeTask, planStep, setPlanStep, onNavigate }) {
  ...
  const steps = [{ title: "Let's make a plan!", subtitle: "Review deadlines and set goals" }, { title: "Your deadlines", subtitle: "This week and upcoming" }, { title: "Set goals", subtitle: "What to accomplish?" }, { title: "Add tasks", subtitle: "Daily and weekly" }]
